...
int main() {
    ...
    Player player(sf::Vector2f(100.f, 480.f)); // Plus haut que le sol à 560
    sf::Vector2f flagPos = level.getFlagPosition();

    // === LIMITE MONDE ===
    const float WORLD_LEFT_BOUND = 0.f;
    const float WORLD_RIGHT_BOUND = 3500.f; // Doit matcher avec levelWidth
    const float WORLD_TOP_BOUND = 0.f;
    const float WORLD_BOTTOM_BOUND = 600.f; // Hauteur de la fenêtre

    // --- FlagPole (poteau du drapeau) ---
    sf::RectangleShape flagPoleShape;
    // SFML 3.0: setSize prend un sf::Vector2f
    flagPoleShape.setSize(sf::Vector2f(8.f, 130.f));
    flagPoleShape.setFillColor(sf::Color(160, 160, 160));
    // SFML 3.0: setPosition prend un sf::Vector2f
    ...
    // --- Variables animation fin de niveau ---
    bool flagTouched = false;
    float flagDropY = flagPos.y - 130.f; // Position verticale du drapeau (en haut du poteau)
    // SFML 3.0: utiliser size.y au lieu de height
    float flagTargetY = flagPos.y - flagSprite.getGlobalBounds().size.y / 2.f; // Position cible (bas du poteau)
    float flagSpeed = 120.f; // Vitesse de descente du drapeau

    // --- Texte des pièces ---
    sf::Font font;
    // SFML 3.0: openFromFile au lieu de loadFromFile pour sf::Font
    if (!font.openFromFile("src/assets/font/font.ttf")) {
        std::cerr << "[WARNING] font.ttf introuvable." << std::endl;
    }
    // SFML 3.0: sf::Text constructeur avec font en premier paramètre
    ...
    while (window.isOpen()) {
        ...
        // --- Collision Joueur/Ennemis (Goomba stomp) ---
        for (auto& enemy : enemies) {
            if (!enemy.isAlive()) continue;
            // SFML 3.0: utiliser findIntersection au lieu de intersects
            if (player.getBounds().findIntersection(enemy.getBounds()).has_value()) {
                ...
                // SFML 3.0: utiliser position.y + size.y au lieu de top + height
                ...
                // sinon: dégâts joueur à implémenter si besoin
            }
        }
        ...
        // === CAMERA SUIVANT MARIO, mais limitée ===
        ...
        window.draw(flagPoleShape);  // Poteau
        window.draw(flagSprite);     // Drapeau
